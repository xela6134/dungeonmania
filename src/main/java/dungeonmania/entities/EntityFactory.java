...
import dungeonmania.entities.logical.LightBulbOff;
import dungeonmania.entities.logical.LightBulbOn;
...
import dungeonmania.entities.logical.strategy.AndStrategy;
import dungeonmania.entities.logical.strategy.CoAndStrategy;
import dungeonmania.entities.logical.strategy.OrStrategy;
import dungeonmania.entities.logical.strategy.Strategy;
import dungeonmania.entities.logical.strategy.XorStrategy;
...
public class EntityFactory {
    ...
    public Buildable buildSceptre() {
        int sceptreDurability = config.optInt("sceptre_durability", 1);
        int mindControlDuration = config.optInt("mind_control_duration", 3);
        return new Sceptre(sceptreDurability, mindControlDuration);
    }

    public Buildable buildMidnightArmour() {
        double armourDefence = config.optDouble("midnight_armour_defence", 10);
        double armourAttack = config.optDouble("midnight_armour_attack", 10);
        int armourDurability = config.optInt("midnight_armour_durability", 100);
        return new MidnightArmour(armourDurability, armourAttack, armourDefence);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "bomb":
            ...
            if (jsonEntity.has("logic")) {
                return new Bomb(pos, bombRadius, stringToStrategy(jsonEntity.getString("logic")));
            } else {
        ...
        case "wire":
            return new Wire(pos);
        case "light_bulb_off":
            return new LightBulbOff(pos, stringToStrategy(jsonEntity.getString("logic")));
        case "light_bulb_on":
            return new LightBulbOn(pos, stringToStrategy(jsonEntity.getString("logic")));
        case "switch_door":
            return new SwitchDoor(pos, stringToStrategy(jsonEntity.getString("logic")), false);
        case "sun_stone":
    ...
    private Strategy stringToStrategy(String str) {
        switch (str) {
            case "and":
