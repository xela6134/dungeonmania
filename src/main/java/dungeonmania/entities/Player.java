...
public class Player extends Entity implements Battleable, Overlappable {
    ...
    public void onOverlap(GameMap map, Entity entity) {
        if (entity instanceof Enemy) {
            ...
            map.battleInGame(this, (Enemy) entity);
        }
    }
    ...
    public boolean pickUp(Entity item) {
        ...
        else if (item instanceof SunStone) collectedTreasureCount++;
        return inventory.add((InventoryItem) item);
    }

    public void collect(Entity item, GameMap map) {
        if (pickUp(item)) {
    ...
    public <T> List<T> getInventoryEntities(Class<T> cls) {
        return inventory.getEntities(cls);
    }
    ...
    public <T extends InventoryItem> void use(Class<T> itemType) {
        inventory.removeEntity(itemType, 1);
    }

    public void use(Bomb bomb, GameMap map) {
        inventory.removeEntity(Bomb.class, 1);
    ...
    public void use(Potion potion, int tick) {
        inventory.removeEntity(potion.getClass(), 1);
        queue.add(potion);
        if (inEffective == null) {
            triggerNext(tick);
        }
    ...
    public void remove(InventoryItem item) {
        inventory.removeEntity(item.getClass(), 1);
    }
